...
var index=priority||0;this.countryCodes[dialCode][index]=iso2;},processCountries:function(countryArray,processFunc){var i;for(i=0;i<countryArray.length;i++){countryArray[i]=countryArray[i].toLowerCase();}
...
if(this.options.defaultCountry=="auto"){this._loadAutoCountry();}else{this.autoCountryDeferred.resolve();}},_loadAutoCountry:function(){var that=this;var cookieAutoCountry=window.Cookies?Cookies.get("itiAutoCountry"):"";if(cookieAutoCountry){$.fn[pluginName].autoCountry=cookieAutoCountry;}
...
if(activeListItem.length){this._scrollTo(activeListItem);}
...
return false;},loadUtils:function(path){var that=this;var utilsScript=path||this.options.utilsScript;if(utilsScript){if(!$.fn[pluginName].loadedUtilsScript){$.fn[pluginName].loadedUtilsScript=true;$.ajax({url:utilsScript,complete:function(){$(".intl-tel-input input").intlTelInput("utilsRequestComplete");},dataType:"script",cache:true});}}else{this.utilsScriptDeferred.resolve();}},selectCountry:function(countryCode){countryCode=countryCode.toLowerCase();if(!this.selectedFlagInner.hasClass(countryCode)){this._selectFlag(countryCode,true);this._updateDialCode(this.selectedCountryData.dialCode,false);}},setNumber:function(number,format,addSuffix,preventConversion,isAllowedKey){if(!this.options.nationalMode&&number.charAt(0)!="+"){number="+"+number;}
this._updateFlagFromNumber(number);this._updateVal(number,format,addSuffix,preventConversion,isAllowedKey);},utilsRequestComplete:function(){if(window.intlTelInputUtils){if(this.options.autoFormat&&this.telInput.val()){this._updateVal(this.telInput.val());}
